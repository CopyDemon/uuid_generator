...
export async function POST(request: Request) {
    try {
        ...
        // 不需要存入数据库的计算字段
        const excludeFields = ["total", "ceiling_total", "ceiling_saving"];
        let insertCount = 0;
        ...
        for (const day of spendData) {
            ...
            for (const { category, name, price } of day.items) {
                // 跳过计算字段
                ...
                const query = `
                    INSERT INTO expense.daily_expense (date, item, amount, category) 
                    VALUES (
                        $1, 
                        $2, 
                        $3, 
                        (SELECT id FROM expense.category WHERE category = $4)
                    )
                    ON CONFLICT (date, item)
                    DO UPDATE SET amount = $3;
                `;
                await pool.query(query, [date, name, price, category]);
                insertCount++;
            }
        }
